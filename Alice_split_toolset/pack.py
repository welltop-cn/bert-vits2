    ...
    for folder in tqdm(all_folders, desc="Processing folders", unit="folder"):
        ...
        if os.path.exists(mapping_path):
            ...
            for line in tqdm(lines, desc=f"Processing files in {folder}", unit="file", leave=False):
                ...
                new_mapping_entry = f"Data/{character_name}/audios/wavs/{new_filename}|{character_name}|EN|{text}"
                
                
                # Copy and rename the file
                copyfile(old_filepath, new_filepath)

                with open(os.path.join(dataset_path, "short_character_anno.list"), 'a', encoding='utf-8') as dataset_file:
