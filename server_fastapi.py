...
if __name__ == "__main__":
    ...
    def voice(
        ...
        sdp_ratio: float = Query(0.2, description="SDP/DP混合比"),  # SDP对应随机时长预测，DP对应时长预测
    ...
    def add_model(
        ...
        logger.info(
            f"CHAOS: model_path={model_path}, config_path={config_path}"
        )
        if config_path is None:
            ...
            if os.path.isfile(os.path.join(model_dir, "config.json")):
                ...
                config_path = os.path.join(model_dir, "../config.json")
            # CHAOS: 增加路径
            elif os.path.isfile(os.path.join(model_dir, "../configs/config.json")):
